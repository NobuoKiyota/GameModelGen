...
| **頭部 (Head)** | X: 0.48m, Y: 0.42m, Z: 0.40m | ハイポリUV球 (32x24) ＋ 下膨れ頬変形 ＋ Subdiv Lv2（完全滑らか） |
| **髪型 (Hair)** | 高解像度UV球 (36x24) | 滑らかなおでこ生え際（M字アーチ）＋ 前髪房ウェーブ（ファセットゼロ） |
| **衣装 (Outfit)** | 円錐台 (36 segments) ＋ 袖 | 腕の付け根を包む短い袖（Sleeves）により脇腹の貫通を完全防止 |
| **手先 (Hands)** | スキン半径 0.08m | 指を分けないミトン状・カプセル状の丸い手（Subdiv Lv3） |
| **胴体 (Torso)** | スキン半径 腹: 0.145m, 胸: 0.125m | 釣鐘・円錐台のコロンとしたふくらみ |
| **脚 (Legs)** | スキン半径 0.075m, 足首Z=0.025m | 短く丸い円柱脚（靴の中にしっかり埋め込み接地） |
| **靴 (Shoes)** | 幅 0.10m, 長 0.15m, 高 0.085m | 接地 `Z = 0.0`、底面足音Surface ID連動 |
| **総ポリゴン密度** | 評価頂点数 約40,000 verts | カクカク感・角張りのない映画・ゲーム両対応ハイポリ曲面 |
