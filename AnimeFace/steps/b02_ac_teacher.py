...
def create_ac_body_mesh(materials):
    """
    どうぶつの森風の体型：
    - 首：hair_h02_human.blendの12頂点（Z=-0.3076, 中心(0, 0.007)）に完全合致
    - トルソー：くびれのないコロンとした寸胴（洋梨・円筒型）。Z=-0.3076 〜 -0.62
    - 腕：短く太い、手先は丸い球体（ドラえもん/あつ森の手！）
    - 脚：短くぽてっとした円柱脚。Z=-0.62 〜 -0.88
    - 素足・足先：丸いコロンとした足
    """
    ...
    # どうぶつの森風の寸胴トルソー（Z=-0.3076 〜 -0.62）
    # くびれを無くし、ふっくらした円筒〜少し下膨れの洋梨型
    torso_levels = [
        {"z": -0.34, "rx": 0.105, "ry": 0.105, "cy": 0.007},
        {"z": -0.38, "rx": 0.125, "ry": 0.115, "cy": 0.005},
        # 肩〜胸（丸っこい）
        {"z": -0.43, "rx": 0.145, "ry": 0.125, "cy": 0.002},
        # お腹中央（ぽてっと丸い寸胴）
        {"z": -0.50, "rx": 0.155, "ry": 0.135, "cy": 0.005},
        # ウエスト下部〜腰（くびれず丸みを維持）
        {"z": -0.56, "rx": 0.160, "ry": 0.140, "cy": 0.008},
        # 股下・ヒップ底面
        {"z": -0.62, "rx": 0.150, "ry": 0.130, "cy": 0.010},
    ]
    ...
    # --- どうぶつの森風の短い脚（Z=-0.60 〜 -0.88） ---
    leg_levels = [
        # 付け根
        {"z": -0.60, "cx": -0.075, "cy": 0.010, "rx": 0.060, "ry": 0.060},
        {"z": -0.68, "cx": -0.075, "cy": 0.010, "rx": 0.055, "ry": 0.055},
        {"z": -0.76, "cx": -0.075, "cy": 0.008, "rx": 0.050, "ry": 0.050},
        # 足首
        {"z": -0.83, "cx": -0.075, "cy": 0.008, "rx": 0.046, "ry": 0.046},
        # 足底（底面）
        {"z": -0.88, "cx": -0.075, "cy": -0.005, "rx": 0.048, "ry": 0.065},
    ]
    leg_rings = []
    ...
    # --- どうぶつの森風の短い腕＆丸い手（Tポーズ） ---
    # 肩から手先まで短く、手先は丸い球体（あつ森の手）
    arm_levels = [
        # 肩
        {"x": -0.13, "cy": 0.000, "cz": -0.43, "ry": 0.052, "rz": 0.052},
        # 腕中央
        {"x": -0.22, "cy": 0.000, "cz": -0.44, "ry": 0.046, "rz": 0.046},
        # 手首
        {"x": -0.30, "cy": 0.000, "cz": -0.45, "ry": 0.040, "rz": 0.040},
        # 丸い手（球状のふくらみ）
        {"x": -0.36, "cy": 0.000, "cz": -0.45, "ry": 0.048, "rz": 0.048},
        # 手先先端
        {"x": -0.41, "cy": 0.000, "cz": -0.45, "ry": 0.025, "rz": 0.025},
    ]
    arm_rings = []
