...
- UIボタン「💥 木っ端微塵に破壊してUE用FBX一括出力」は `<export_folder>/<扉名>_UE/` に3ファイル出力: `_Frame.fbx`(石枠static) / `_LeavesIntact.fbx`(無傷の扉static。破壊メッシュのrestは破片の隙間でヒビ状に見えるため破壊時に入れ替える用) / `_Destruction.fbx`(スケルタル＋ボーンアニメ)。FBXは -Y forward / Z up、**`FBX_SCALE_NONE`**（UEでは1m=100cm。`FBX_SCALE_UNITS`/`ALL`は1cmで入り扉が1/100になる。UE 5.3.2で実測して修正済み）、`add_leaf_bones=False`、`use_armature_deform_only=False`（rootを必ず出力）、Armatureは出力時のみ一時的に"Armature"名・原点へ。
...
- **UE 5.3.2 実機検証（リモートPython実行で実施）**: UEエディタのPythonリモート実行(UDP 6766)経由でスクリプトを実行できた（MCP不要）。`create_door_blueprint.py` で `/Game/DoorDestruction/` に Frame/LeavesIntact(StaticMesh)、Destruction(SkeletalMesh)、AnimSequence、Skeleton、`BP_OpDoor` が生成され、BPコンポーネント設定値も想定どおり。**検証で判明した不具合: 当初のFBXが`FBX_SCALE_UNITS`で扉が1/100サイズだった → `FBX_SCALE_NONE`に修正**。アニメの移動量はBlender再インポート(91.9)とUE(92.2)で一致。UE Python 5.3 にはBPノード作成APIが無く `Break` は手動。まだ未検証: `Break` 関数/Eventトラック発火、旧シーケンススクリプト。
- (下記のうちUEインポート/スケール/BP構成は上で検証済み)**未検証**: UEでのインポート結果（ルートボーン名・スケール・軸・アニメ取り込み）、UE Pythonスクリプト全般、Visibilityトラックのキー設定。エラーはOutput Logの文面をもらって修正する前提。
