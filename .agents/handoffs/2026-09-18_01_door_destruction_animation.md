# [2026-09-18] Claude Code — procedural_rock_studio「西洋風アーチ扉」DOORプリセット＋破壊アニメーション
...
4. 密着した破片は摩擦で固まって動かないため、**衝撃点からキネマティック（直接アニメーション）で強制的に弾き飛ばし**、`impact_frames`後にRigid Body(ACTIVE, CONVEX_HULL)へ引き渡して重力・床/石枠(PASSIVE)との衝突で自然に落下・転がらせる
5. `sample_step`間隔でフレームをサンプリングし、各破片の`matrix_world`を記録
