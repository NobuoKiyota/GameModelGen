...
def build_chibi_hair_mesh(context, name="Chibi_Hair", gender="BOY", hair_style="SHORT", head_center=(0, 0, 0.72), head_size=(0.48, 0.42, 0.40)):
    ...
    if hair_style == "SHORT":
        ...
        # ふんわりボブ: フェイスラインを包む丸い内巻き
        for v in bm.verts:
            if v.co.z < hc.z + 0.05:
                v.co.x *= 1.10
                v.co.y *= 1.06
    elif hair_style == "SPIKY":
        # ツンツンヘア: 頭頂部〜後頭部のハネ毛スパイク
        spikes = [
        ...
        # もこもこアフロ: まん丸でボリューミーなポップヘア（顔正面をくり抜き）
...
def build_chibi_outfit_mesh(context, name="Chibi_Outfit", gender="BOY", outfit_type="T_SHIRT", base_z=0.05, body_height=0.63):
    ...
    if outfit_type == "ONE_PIECE" or gender == "GIRL":
        ...
        # 左右の短い袖（Sleeves: 12 segments - 肩から腕に沿って自然に下向きに伸びる構造）
        sin_theta = 0.36   # 外側への傾き
        cos_theta = 0.933  # 下向きの傾き
        sleeve_len = 0.125
        ...
        for side in (-1.0, 1.0):
            ...
            # radius1 (Z = -len/2, 袖口): 0.100
            # radius2 (Z = +len/2, 肩付け根): 0.125
            res_sl = bmesh.ops.create_cone(
                bm, cap_ends=False, segments=12,
                radius1=0.100, radius2=0.125, depth=sleeve_len
            )
            for v in res_sl['verts']:
                vx = v.co.x
                vy = v.co.y
                vz = v.co.z
                # vz < 0 (袖口) のときに外側(+side)かつ下(-Z)へ
                # vz > 0 (肩付け根) のときに内側(-side)かつ上(+Z)へ
